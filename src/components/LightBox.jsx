        ...
        <Wrapper onClick={toggleIsOpen}>
            ...
            {isOpen ? (
                <div
                    ...
                    className="mx-auto flex flex-col md:flex-row absolute top-0 left-0 w-screen h-screen z-50 cursor-pointer bg-black"
                >
                    <img
                        src={data.src}
                        alt={data.alt}
                    ...
                    <div className="bg-white m-1 p-1 flex flex-col flex-wrap place-self-center">
                        <p>{data.alt}</p>
                        <p>Longitude: {data.solidImage.metadata.location?.longitude} </p>
                        <p>Latitude: {data.solidImage.metadata.location?.latitude} </p>
                        <p>{data.solidImage.metadata.createdDate?.toString()}</p>
                        {/*<p>{data.desc}</p>*/}
                    </div>
                </div>
            ) : null}
